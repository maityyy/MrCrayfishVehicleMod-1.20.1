...
public abstract class AbstractLandVehicleRenderer<T extends LandVehicleEntity & EntityRayTracer.IEntityRayTraceable> extends AbstractPoweredRenderer<T>
{
            ...
            if(properties.getFrontAxelVec() != null && properties.getRearAxelVec() != null)
            {
                matrixStack.pushPose();
                {
                    ...
                    matrixStack.pushPose();
                    {
                        Vec3 frontAxelVec = properties.getFrontAxelVec();
                        frontAxelVec = frontAxelVec.scale(0.0625);
                        Vec3 nextFrontAxelVec = new Vec3(0, 0, vehicle.getSpeed() / 20F).yRot(vehicle.renderWheelAngle * 0.017453292F);
                    ...
                    matrixStack.pushPose();
                    {
                        ...
                        rearAxelVec = rearAxelVec.add(Vec3.directionFromRotation(0, (float) deltaYaw).scale(vehicle.getSpeed() / 20F));
                        matrixStack.translate(rearAxelVec.x, 0, rearAxelVec.z);
                        this.renderSteeringLine(matrixStack, 0xFFDD00);
                    }
                    matrixStack.popPose();
                    ...
                    matrixStack.pushPose();
                    {
                        Vec3 nextFrontAxelVec = new Vec3(0, 0, vehicle.getSpeed() / 20F).yRot(vehicle.wheelAngle * 0.017453292F);
                        nextFrontAxelVec = nextFrontAxelVec.add(properties.getFrontAxelVec().scale(0.0625));
                        Vec3 nextRearAxelVec = new Vec3(0, 0, vehicle.getSpeed() / 20F);
    ...
    protected void renderWheel(@Nullable T vehicle, Wheel wheel, ItemStack stack, BakedModel model, float partialTicks, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, int light)
    {
        ...
        {
            float wheelAngle = Mth.lerp(partialTicks, this.prevWheelAngleProperty.get(vehicle), this.wheelAngleProperty.get(vehicle));
            matrixStack.mulPose(Axis.YP.rotationDegrees(wheelAngle));
        }
        if(vehicle != null && vehicle.isMoving())
        {
            matrixStack.mulPose(Axis.XP.rotationDegrees(-wheel.getWheelRotation(vehicle, partialTicks)));
        }
